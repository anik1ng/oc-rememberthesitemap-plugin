<?php namespace ANIKIN\RememberTheSitemap\FormWidgets;
...
class SitemapItems extends FormWidgetBase
{
    ...
    public $referenceRequiredMessage = 'anikin.rememberthesitemap::lang.item.reference_required';

    public $urlRequiredMessage = 'anikin.rememberthesitemap::lang.item.url_required';

    public $cmsPageRequiredMessage = 'anikin.rememberthesitemap::lang.item.cms_page_required';
    ...
    public function prepareVars()
    {
        ...
        $widgetConfig = $this->makeConfig('$/anikin/rememberthesitemap/classes/definitionitem/fields.yaml');
     ...
     * @param \Anikin\RememberTheSitemap\Classes\SitemapItem $item Specifies the sitemap item
     * @return string
     */
    protected function getReferenceDescription($item)
    {
        ...
        if (isset($this->typeListCache[$item->type])) {
            ...
            $result = trans('anikin.rememberthesitemap::lang.item.unknown_type');
        }
    ...
    protected function findReferenceName($search, $typeOptionList)
    {
        ...
        if (!strlen($result)) {
            $result = trans('anikin.rememberthesitemap::lang.item.unnamed');
        }
    ...
    protected function getSitemapItemTitle($itemInfo)
    {
        if (is_array($itemInfo)) {
            if (!array_key_exists('title', $itemInfo) || !strlen($itemInfo['title'])) {
                return trans('anikin.rememberthesitemap::lang.item.unnamed');
            }

            return $itemInfo['title'];
        }

        return strlen($itemInfo) ? $itemInfo : trans('anikin.rememberthesitemap::lang.item.unnamed');
    }
}
